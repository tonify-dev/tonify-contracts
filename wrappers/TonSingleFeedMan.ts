...
import { OP_REDSTONE_CHECK_AND_RETURN_PRICE, OP_REDSTONE_WRITE_PRICE } from "../src/config/constants";
...
export class TonSingleFeedMan extends TonContract {
  ...
  async sendCheckAndReturnPrice(
    provider: ContractProvider,
    paramsProvider: ContractParamsProvider
  ) {
    const builder = messageBuilder(OP_REDSTONE_CHECK_AND_RETURN_PRICE);
